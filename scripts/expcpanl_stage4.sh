...
# Do this step, before pcpanl_chkst3log.sh, so that more QPEs might come in 
#   before we check for them. 
if [ $cyc -eq 02 -o $cyc -eq 08 -o $cyc -eq 14 -o $cyc -eq 20 ]; then
  v6date=`$NDATE -2 ${PDY}$cyc`
  $USHpcpanl/pcpanl_make_hrlywgts.sh $v6date
fi

# 
$USHpcpanl/pcpanl_chkst3log.sh 
export err=$?; err_chk
  ...
  if [ -s todo4.$region.${PDY}${cyc} ]; then
    ...
    do
      ...
      if [ $region = conus ]; then
        ...
        $USHpcpanl/pcpanl_st4mosaic.sh $time $ac
        export err=$?; err_chk
      else
        ...
        # end of pcpanl_st4oconus.sh.
        $USHpcpanl/pcpanl_st4oconus.sh $region $time $ac
        export err=$?; err_chk
      fi
    done
  fi 
...
$USHpcpanl/pcpanl_st3log_summary.sh $PDYm9
export err=$?; err_chk
$USHpcpanl/pcpanl_st3log_summary.sh $PDYm7
export err=$?; err_chk
$USHpcpanl/pcpanl_st3log_summary.sh $PDYm5
export err=$?; err_chk
$USHpcpanl/pcpanl_st3log_summary.sh $PDYm3
export err=$?; err_chk
$USHpcpanl/pcpanl_st3log_summary.sh $PDYm1
export err=$?; err_chk
$USHpcpanl/pcpanl_st3log_summary.sh $PDY
export err=$?; err_chk
fi
