...
cp $COMOUT/$RUN.$PDY/toplot4.$date0 .
...
#   cp $COMIN/$RUN.$day/${ST4item}.grb2 $ST4item/.
    cp $COMOUT/$RUN.$day/${ST4item}.grb2 $ST4item/.
    if [ $? -eq 0 ]; then
      # Now make the plot.  Note that ST4item below is the subdir containing
      # $ST4item.grb2:
      echo "$USHpcpanl/pcpanl_plotpcp.sh $ST4item $ST4item.gif $date $ac \"Stage IV\" $region" >>poescript
    else
...
#   Mail -s "pcpanl alert" Jun.Du@noaa.gov < $DATA/emailmsg.txt
...
mpiexec --cpu-bind core cfp ./poescript
export err=$?; err_chk
