  ...
  do
    ...
    then
      ...
      if [ -s $weights -a -s $QPE6h ]; then
        $USHpcpanl/pcpanl_st4_qpe6h_to_1h.sh $v6date $rid
      fi
...
# acc=06h/24h?  Make 6h/24 sums for select RFC
...
then
  # Note that we are assuming that ush/pcpanl_st4mosaic.sh and pcpanl_st4oconus.sh
...
if test $SENDDBN = 'YES'
then
  $SIPHONROOT/bin/dbn_alert MODEL PCPANL $job $COMOUT/${RUN}.$day/st4_conus.$date.$ac.grb2
fi
